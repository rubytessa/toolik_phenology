...
wdir <- "/home/ruby/unispec-sc/20220610/" #this is the path to the folder containing your (1) files and (2) field key
setwd(wdir)
# Create vector of ".spu" files 
spu_files <- list.files(path = getwd(), pattern = ".spu$", full.names = T, recursive=T)
...
ggplot(df, aes(x=Wavelength, y = Ratio)) +
  geom_line(aes(color = Species, group = FileNum)) + 
  facet_wrap(vars(Plant_Num))
